...
function CalendarCell({ state, date }: CalendarCellProps) {
	...
	return (
		<td
			...
			className={cn(
				cellProps.className,
				"relative p-0 text-center text-sm focus-within:relative focus-within:z-20 [&:has([aria-selected])]:bg-accent first:[&:has([aria-selected])]:rounded-none last:[&:has([aria-selected])]:rounded-none",
			)}
		>
			<Button
				{...buttonProps}
